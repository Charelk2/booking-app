...
The global search bar and its compact pill appear only on the home page and the service providers listing page. Service provider detail pages and other routes omit these elements for a cleaner layout. On mobile, the header always shows the compact pill next to the Booka logo; tapping it opens the full search accordion. On the service providers listing page, the header loads directly in its compact pill state, preserving any category, date, and location selections from the URL and showing the filter icon beside the pill for quick refinement. Clicking the pill now expands the full SearchBar above the filter controls, and the compact pill mirrors the collapsed SearchBar by displaying any selected category, dates, and location when the full bar is hidden. For addresses, the pill shows only the street name to keep things concise.
