    ...
    try {
      ...
      if (usedFallback) {
        try {
          ...
          const view = `/${serviceProvider?.slug || serviceProviderId}`;
          const card = {
            inquiry_sent_v1: {
              title,
              cover,
            ...
            <div className="mx-auto max-w-6xl md:flex bg-white">
              ...
              <aside className="md:w-2/5 md:flex md:flex-col bg-white md:sticky md:self-start md:border-gray-100 p-0" style={{ top: 'var(--sp-sticky-top)' }}>
                <div className="h-[calc(100vh-var(--sp-sticky-top))] flex flex-col">
                  <div ref={leftRef} className="flex-1 overflow-y-auto p-4 scrollbar-hide">
                    <div className="relative h-56 overflow-hidden rounded-3xl shadow-lg" role="img" aria-label="Cover photo">
                      {coverPhotoUrl ? (
              ...
              <section className="md:w-3/5 md:sticky md:self-start p-0" style={{ top: 'var(--sp-sticky-top)' }}>
                ...
                <section aria-labelledby="reviews-heading-desktop" className="group pb-10">
                  ...
                  <h2 id="reviews-heading-desktop" className="mt-12 text-lg font-bold text-gray-800 mb-3 hidden">Reviews</h2>
                  {displayReviews.length ? (
                    <>
                      <ReviewSummary reviews={displayReviews} />
                      ...
                      {displayReviews.length > 6 && (
                        <div className="mt-2">
                          <button type="button" className="w-full inline-flex items-center justify-center rounded-md bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-200 transition" onClick={() => setIsAllReviewsOpen(true)}>
                            Show all reviews
                          </button>
                        </div>
                      )}
