            ...
            for (let index = 0; index < attachments.length; index += 1) {
              ...
              onOptimisticMessage(
                mkOptimistic(tempId, {
                  ...
                  reply_to_message_id: replyId,
                  reply_to_preview:
                    replyId && replyTarget ? replyTarget.content.slice(0, 120) : null,
              ...
              try {
                ...
                if (!(file.type || "").toLowerCase().startsWith("audio/") && previewUrl) {
                  try {
                ...
                onError?.(
                  `Failed to send attachment ${file.name || ""}. ${err?.message || ""}`.trim()
                );
              } finally {
                onUploadProgress?.(tempId, 0);
                replyId = null;
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 flex flex-wrap items-center gap-2 overflow-hidden">
                  {imagePreviewUrls.map((u, i) => (
                    <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden ring-1 ring-black/10 dark:ring-white/10 bg-white/40 dark:bg-white/10">
            ...
            <GlassBar className="p-2">
              <div className="flex items-center gap-3">
                ...
                (attachmentFile.type.startsWith("audio/") ||
                  /\.(webm|mp3|m4a|ogg|wav)$/i.test(attachmentFile.name || "")) ? (
                  <>
                    <DocumentIcon className="w-7 h-7 text-indigo-600" />
                    <span className="text-xs font-medium">
                      {attachmentFile.name} ({Math.round((attachmentFile.size || 0) / 1024)} KB)
                    </span>
                  </>
                ) : attachmentFile &&
                  (attachmentFile.type.startsWith("video/") ||
                    /\.(mp4|mov|webm|mkv|m4v)$/i.test(attachmentFile.name || "")) ? (
                  <>
                    <video className="w-48 rounded-lg ring-1 ring-black/10 dark:ring-white/10" controls src={attachmentPreviewUrl} preload="metadata" />
        ...
        <div className="px-2 pt-1.5 pb-2">
          <GlassBar className="px-2 py-1.5">
            <form ref={formRef} onSubmit={handleSendMessage} className="flex items-end gap-1.5">
              ...
              {/* Upload */}
              <label htmlFor="file-upload" aria-label="Upload attachment" className="cursor-pointer">
                <GlassIconButton title="Upload">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                       strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15"/>
                  </svg>
              ...
              {/* Emoji */}
              <GlassIconButton aria-label="Add emoji" title="Emoji" onClick={() => setShowEmojiPicker((p) => !p)}>
              ...
              {/* Send (SOLID icon → always visible) */}
              <button
                type="submit"
                aria-label="Send message"
                title="Send"
                className={[
                  "flex-shrink-0 rounded-full w-9 h-9 grid place-items-center",
