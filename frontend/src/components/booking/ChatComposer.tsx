      ...
      if (attachments.length > 0) {
        ...
        for (let index = 0; index < attachments.length; index += 1) {
          ...
          try {
            ...
            const res = await postMessageToBookingRequest(bookingRequestId, payload);
            onFinalizeMessage(tempId, res.data);
            // Free preview URLs after some time for non-audio (keep audio slightly longer if needed elsewhere)
            if (!(file.type || '').toLowerCase().startsWith('audio/') && previewUrl) {
              try { setTimeout(() => URL.revokeObjectURL(previewUrl), 4000); } catch {}
            }
          } catch (err: any) {
