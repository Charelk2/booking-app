...
const ChatComposer = React.forwardRef<HTMLTextAreaElement, ChatComposerProps>(
  function ChatComposer(props, forwardedRef) {
    ...
    const openEmoji = async () => {
      ...
      if (!emojiData) {
        try {
          const modData = await import("@emoji-mart/data");
          setEmojiData((modData as any).default || (modData as any));
        } catch {}
      }
      ...
      try {
        if (trimmed) {
          ...
          if (!navigator.onLine) {
            ...
            try {
              // Include a client request id so WS echo can flip to 'sent' fast
              const clientRequestId = (typeof crypto !== 'undefined' && (crypto as any).randomUUID)
                ? (crypto as any).randomUUID()
                : `cid:${Date.now()}:${Math.floor(Math.random() * 1e6)}`;
              const res = await postMessageToBookingRequest(bookingRequestId, payload, { clientRequestId });
