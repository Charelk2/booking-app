  ...
  try {
    ...
    if (key.startsWith('event_finished_v1')) {
      ...
      const subtitleClient = t(
        ...
        'If anything was not as expected, you can report a problem from this chat within 12 hours.',
      );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-600">
                  {isProvider ? subtitleArtist : subtitleClient}
                </div>
              </div>
            </div>
            <div className="flex gap-2">
              {isProvider && (
                ...
                >
                  {t('system.markCompleted', 'Mark as completed')}
                </button>
              )}
              <button
                type="button"
              ...
              >
                {t('system.reportProblem', 'Report a problem')}
              </button>
            </div>
          </div>
        </div>
    ...
    if (key === 'event_auto_completed_v1') {
      return (
        <div className="my-2 w-full flex justify-center">
          <div className="text-[12px] text-gray-700 bg-gray-100 border border-gray-200 px-2 py-1 rounded">
            {t(
            ...
            <button
              type="button"
              className="inline-flex items-center rounded px-1.5 py-0.5 text-[11px] font-medium text-gray-700 hover:bg-gray-200"
              onClick={() => {
                try {
                  onOpenReview?.();
                } catch {}
    ...
    if (key.startsWith('dispute_opened_v1')) {
      return (
        <div className="my-2 w-full flex justify-center">
          <div className="text-[12px] text-gray-700 bg-yellow-50 border border-yellow-200 px-2 py-1 rounded">
            {t(
              'system.disputeOpened',
              'A problem has been reported for this event. Our team will review the details.',
            )}
          </div>
        </div>
      );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-semibold">
                  {t('system.reviewInviteTitle', 'How was your event?')}
                </div>
                <div className="text-xs text-gray-600">
                  {t('system.reviewInviteBody', 'Leave a rating and short review.')}
                </div>
              </div>
            </div>
            <button
