...
export default function AddServiceModalSpeaker({
  isOpen,
  onClose,
  onServiceSaved,
  service,
  ...
  const defaults: SpeakerForm = service
    ? {
        title: service.title,
        price: service.price,
        topic: (service.details as any)?.topic ?? "",
      }
    : empty;

  const steps: WizardStep<SpeakerForm>[] = [
    {
      label: "Details",
      fields: ["title", "price", "topic"],
      render: ({ form }) => (
        <div className="space-y-4">
          <h2 className="text-xl font-semibold">Speaker Details</h2>
          <TextInput
            label="Title"
