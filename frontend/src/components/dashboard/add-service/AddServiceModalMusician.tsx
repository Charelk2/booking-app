...
// ────────────────────────────────────────────────────────────────────────────────
// Tech Rider model (stored under service.details.tech_rider)
type PowerPhase = "single" | "three";

interface TechChannel {
  ch: number;           // 1-based channel number
  source: string;       // e.g., "Lead Vox", "Acoustic DI", "Kick"
  mic?: string;         // preferred mic model or "Any dynamic"
  di?: boolean;         // DI required
  phantom?: boolean;    // +48V needed
  notes?: string;
}

interface TechMonitoring {
  mixes: number;           // number of monitor mixes
  wedges_ok: boolean;      // can use wedges
  iem_ok: boolean;         // can use IEMs
...
interface TechStage {
  min_width_m?: number | "";
  min_depth_m?: number | "";
  cover_required: boolean;  // for outdoor
  stage_plot_url?: string;  // optional image URL
}
...
interface TechFOH {
  min_channels: number | "";
  console_preference?: string;   // e.g., "M32, CL5, SQ5 (USB playback)"
  fx_requirements?: string;      // e.g., "2x reverb, 1x delay"
  playback_sources?: string;     // e.g., "Laptop (TRS), Phone (3.5mm), USB"
}

interface TechBackline {
  required_items: string[];      // e.g., ["Full drum kit", "Bass amp", "Guitar amp"]
  optional_items?: string[];     // nice-to-haves
  notes?: string;
}

interface TechRider {
  lineup_label?: string;         // e.g., "Solo", "Duo", "4-piece band"
  ...
  pdf_url?: string | null;       // mirror for convenience (still upsert separately)
}
...
interface ServiceFormData {
  ...
  title: string;
  description: string;
  duration_label?: string; // free-text label like "60–90 min"
...
const emptyTechRider: TechRider = {
  ...
  channels: [
    { ch: 1, source: "Lead Vocal", mic: "Any cardioid dynamic (SM58/935)", di: false, phantom: false },
  ...
  foh: { min_channels: 8, console_preference: "Any pro digital (M32/X32/CL/SQ)", fx_requirements: "1x reverb, 1x delay", playback_sources: "Laptop TRS or 3.5mm" },
...
export default function AddServiceModalMusician({
  ...
  const editingDefaults = useMemo<ServiceFormData>(() => {
    ...
    return {
      ...
      tech: {
        ...emptyTechRider,
        ...existingTech,
        channels: Array.isArray(existingTech.channels) && existingTech.channels.length > 0
  ...
  // Channels field array for tech rider
  ...
  const onSubmit: SubmitHandler<ServiceFormData> = async (data) => {
    ...
    try {
      ...
      // Media (first file → base64 data URL)
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3 rounded-md border p-3">
                              ...
                              {watchSoundMode === "artist_provides_variable" && (
                                <>
                                  ...
                                  <p className="text-xs text-gray-600">These help us pre-estimate total costs for clients before final venue details.</p>
                                </>
                              )}

                              {watchSoundMode === "external_providers" && (
                                <CollapsibleSection title="Preferred external providers per city" open className="border">
                          ...
                          <div className="space-y-4 rounded-md border p-3">
                            ...
                            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                              <TextInput
                                label="Lineup label"
                                placeholder="e.g., Solo / Duo / 4-piece band"
                                {...register("tech.lineup_label")}
                              />
                              <TextInput
                                label="Minimum FOH channels"
                                type="number"
                                {...register("tech.foh.min_channels", { valueAsNumber: true })}
                              />
                              <TextInput
                                label="Console preference"
                                placeholder="e.g., M32/X32/CL/SQ"
                                {...register("tech.foh.console_preference")}
                              />
                            </div>

                            {/* Inputs / Channels */}
                            <CollapsibleSection title="Inputs / Channels" open className="border">
                              <div className="mt-2">
                                <div className="grid grid-cols-12 gap-2 text-xs font-medium text-gray-600">
                                  <div className="col-span-1">Ch</div>
                                  <div className="col-span-3">Source</div>
                                  <div className="col-span-3">Mic preference</div>
                                  <div className="col-span-1">DI</div>
                                  <div className="col-span-1">48V</div>
                                ...
                                <div className="mt-2">
                                  <button
                                    ...
                                    onClick={() => appendChannel({ ch: (tech?.channels?.length || 0) + 1, source: "", mic: "", di: false, phantom: false, notes: "" })}
                                  >
                                    + Add channel
                                  </button>
                                </div>
                            ...
                            {/* Monitoring */}
                            <CollapsibleSection title="Monitoring" open={false} className="border">
                              <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                                <TextInput
                                  label="Monitor mixes needed"
                                  type="number"
                                  {...register("tech.monitoring.mixes", { valueAsNumber: true })}
                                />
                                <div className="flex items-end gap-2">
                                  <ToggleSwitch
                                    checked={!!watch("tech.monitoring.wedges_ok")}
                                    onChange={(v) => setValue("tech.monitoring.wedges_ok", v, { shouldDirty: true })}
                                    label="Wedges OK"
                                  />
                                </div>
                                <div className="flex items-end gap-2">
                                  <ToggleSwitch
                                    checked={!!watch("tech.monitoring.iem_ok")}
                                    onChange={(v) => setValue("tech.monitoring.iem_ok", v, { shouldDirty: true })}
                                    label="IEMs OK"
                                  />
                                </div>
                                <div className="flex items-end gap-2">
                                  <ToggleSwitch
                                    checked={!!watch("tech.monitoring.talkback_required")}
                                    onChange={(v) => setValue("tech.monitoring.talkback_required", v, { shouldDirty: true })}
                                    label="Talkback required"
                                  />
                                </div>
                              </div>
                            </CollapsibleSection>
                            ...
                            {/* Stage & Power */}
                            <CollapsibleSection title="Stage & Power" open={false} className="border">
                              <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                                ...
                                <div className="flex items-end gap-2">
                                  <ToggleSwitch
                                    checked={!!watch("tech.stage.cover_required")}
                                    onChange={(v) => setValue("tech.stage.cover_required", v, { shouldDirty: true })}
                                    label="Cover required (outdoor)"
                                  />
                                </div>
                                <TextInput
                                  label="Stage plot image URL (optional)"
                                  placeholder="https://…/stageplot.png"
                                  {...register("tech.stage.stage_plot_url")}
                                />
                              </div>
                            ...
                            {/* FOH & Playback */}
                            <CollapsibleSection title="FOH & Playback" open={false} className="border">
                              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                            ...
                            {/* Backline */}
                            <CollapsibleSection title="Backline" open={false} className="border">
                              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                <TextArea
                                  label="Required items (one per line)"
                                  rows={3}
                                  value={(watch("tech.backline.required_items") || []).join("\n")}
                                  onChange={(e) => setValue("tech.backline.required_items", e.target.value.split("\n").map((s) => s.trim()).filter(Boolean), { shouldDirty: true })}
                                />
                                <TextArea
                                  label="Optional items (one per line)"
                                  rows={3}
                                  value={(watch("tech.backline.optional_items") || []).join("\n")}
                                  onChange={(e) => setValue("tech.backline.optional_items", e.target.value.split("\n").map((s) => s.trim()).filter(Boolean), { shouldDirty: true })}
                                />
                              </div>
                              <TextArea
                                label="Backline notes"
                                rows={2}
                                {...register("tech.backline.notes")}
                              />
                            </CollapsibleSection>
                                  ...
                                  <div className="rounded-md border p-2">
                                    ...
                                    <ul className="mt-1 list-disc pl-4">
                                      {(watch("sound_city_prefs") || []).map((r, i) => (
                                        <li key={i}>{r.city || "City?"}: {(r.provider_ids || []).length} provider(s) selected</li>
                                      ))}
                                    </ul>
                                  </div>
                                )}
