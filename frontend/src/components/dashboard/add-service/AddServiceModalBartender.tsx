...
export default function AddServiceModalBartender({
  isOpen,
  onClose,
  onServiceSaved,
  service,
  ...
  const empty: BartenderForm = {
    title: "",
    price: "",
    signature_drink: "",
  };
  const defaults: BartenderForm = service
    ? {
        title: service.title,
        price: service.price,
        signature_drink: (service.details as any)?.signature_drink ?? "",
      }
    : empty;

  const steps: WizardStep<BartenderForm>[] = [
    {
      label: "Details",
      fields: ["title", "price", "signature_drink"],
      render: ({ form }) => (
        <div className="space-y-4">
          <h2 className="text-xl font-semibold">Bartender Details</h2>
          <TextInput
            label="Title"
          ...
          <TextInput
            ...
            {...form.register("signature_drink", { required: true })}
          />
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: ({ form, thumbnails }) => (
        <div className="space-y-2">
          ...
          <p>{form.getValues("signature_drink")}</p>
          {thumbnails.length > 0 && (
  ...
  const toPayload = (
    ...
    media_url: mediaUrl ?? "",
    duration_minutes: 60,
    details: { signature_drink: data.signature_drink },
  });
