...
export default function AddServiceModalWeddingVenue({
  isOpen,
  onClose,
  onServiceSaved,
  service,
  ...
  const empty: WeddingVenueForm = {
    title: "",
    price: "",
    capacity: "",
  };
  const defaults: WeddingVenueForm = service
    ? {
        title: service.title,
        price: service.price,
        capacity: (service.details as any)?.capacity ?? "",
      }
    : empty;

  const steps: WizardStep<WeddingVenueForm>[] = [
    {
      label: "Details",
