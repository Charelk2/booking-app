...
export default function AddServiceModalVideographer({
  isOpen,
  onClose,
  onServiceSaved,
  service,
  ...
  const empty: VideographerForm = {
    title: "",
    price: "",
    video_style: "",
  };
  const defaults: VideographerForm = service
    ? {
        title: service.title,
        price: service.price,
        video_style: (service.details as any)?.video_style ?? "",
      }
    : empty;

  const steps: WizardStep<VideographerForm>[] = [
    {
      label: "Details",
      fields: ["title", "price", "video_style"],
      render: ({ form }) => (
        <div className="space-y-4">
          <h2 className="text-xl font-semibold">Videographer Details</h2>
          <TextInput
            label="Title"
          ...
          <TextInput
            label="Video Style"
            {...form.register("video_style", { required: true })}
          />
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: ({ form, thumbnails }) => (
        <div className="space-y-2">
          ...
          <p>{form.getValues("video_style")}</p>
          {thumbnails.length > 0 && (
  ...
  const toPayload = (
    ...
    media_url: mediaUrl ?? "",
    duration_minutes: 60,
    details: { video_style: data.video_style },
  });
  ...
  return (
    <BaseServiceWizard
      ...
      toPayload={toPayload}
      serviceCategorySlug="videographer"
    />
  );
}
