...
type MusicianCustomSongFlowProps = {
  isOpen: boolean;
  onClose: () => void;
...
export default function MusicianCustomSongFlow({
  isOpen,
  onClose,
  onServiceSaved,
  service,
}: MusicianCustomSongFlowProps) {
  const csConfig = SERVICE_TYPE_REGISTRY.custom_song;
  ...
  const lengthOptions = useMemo(
    () =>
      csConfig.fields.find((f) => f.key === "base_length_sec")?.options || [],
    [csConfig.fields],
  );
  const deliveryFormats = useMemo(
    () =>
      csConfig.fields.find((f) => f.key === "delivery_format")?.options || [],
    [csConfig.fields],
  );
            ...
            >
              ...
              <div className="flex w-full flex-none flex-col justify-between overflow-y-auto bg-gray-50 p-6 md:w-1/5">
                <Stepper
                  ...
                  onStepClick={setStep}
                  ariaLabel="Add custom song service progress"
                  className="space-y-4"
                    ...
                    <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                            {deliveryFormats.map((opt) => (
                              <button
                                ...
                                className={clsx(
                                  "rounded-md border px-3 py-2 text-sm",
                                  state.typeFields.delivery_format === opt.value
                                    ? "border-[var(--brand-color)] bg-[var(--brand-color)]/10"
                                    : "border-gray-200 hover:border-gray-300",
                                )}
                                onClick={() =>
                      ...
                      <div className="rounded-md border border-gray-100 p-3">
                        <ToggleSwitch
                          ...
                          checked={Boolean(state.typeFields.includes_master)}
                          onChange={(v) =>
                            actions.setTypeField("includes_master", v)
                          }
                        />
                        <p className="mt-1 text-xs text-gray-500">
                          Toggle on if you’ll deliver master-quality files.
                        </p>
                      </div>
                    </div>
                  )}
