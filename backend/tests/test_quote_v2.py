...
def test_create_and_accept_quote():
    ...
    booking = api_quote.accept_quote(quote.id, db, current_user=client)
...
def test_read_accepted_quote_has_booking_id():
    ...
    quote = api_quote.create_quote(quote_in, db, current_user=artist)
    booking_simple = api_quote.accept_quote(quote.id, db, current_user=client)
    db_booking = db.query(Booking).filter(Booking.quote_id == quote.id).first()
    assert db_booking is not None
    result = api_quote.read_quote(quote.id, db, current_user=client)
...
def test_read_quote_not_found():
    ...
    db.refresh(user)
    try:
        api_quote.read_quote(999, db, current_user=user)
...
def test_accept_quote_logs_db_error(monkeypatch, caplog):
    ...
    monkeypatch.setattr(api_quote.crud_quote, "get_quote", fake_get_quote)
    monkeypatch.setattr(api_quote.crud_quote, "accept_quote", fail_accept)
    caplog.set_level(logging.ERROR, logger=api_quote.logger.name)

    with pytest.raises(HTTPException) as exc_info:
        api_quote.accept_quote(1, db, current_user=client)
...
def test_accept_quote_booking_failure(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        api_quote.crud_quote,
        "create_booking_from_quote_v2",
        fail_create_booking,
    )
    caplog.set_level(logging.ERROR, logger="app.crud.crud_quote")

    with pytest.raises(HTTPException) as exc_info:
        api_quote.accept_quote(quote.id, db, current_user=client)
...
def test_create_quote_error_logs_and_response(monkeypatch, caplog):
    ...
    monkeypatch.setattr(api_quote.crud_quote, "create_quote", fail_create)
    caplog.set_level(logging.ERROR, logger=api_quote.logger.name)
    with pytest.raises(HTTPException) as exc_info:
        api_quote.create_quote(quote_in, db, current_user=artist)
...
def test_accept_quote_value_error_logs(monkeypatch, caplog):
    ...
    monkeypatch.setattr(api_quote.crud_quote, "get_quote", fake_get_quote)
    monkeypatch.setattr(api_quote.crud_quote, "accept_quote", fail_accept)
    caplog.set_level(logging.WARNING, logger=api_quote.logger.name)
    with pytest.raises(HTTPException) as exc_info:
        api_quote.accept_quote(10, db, current_user=client)
...
def test_accept_quote_supplies_missing_service_id():
    ...
    quote = api_quote.create_quote(quote_in, db, current_user=artist)
    booking = api_quote.accept_quote(quote.id, db, current_user=client, service_id=service.id)
...
def test_expire_pending_quotes():
    ...
    quote = api_quote.create_quote(quote_in, db)
...
def test_scheduler_posts_system_message():
    ...
    with freeze_time("2024-01-01"):
        ...
        api_quote.create_quote(quote_in, db)
...
def test_decline_quote():
    ...
    quote = api_quote.create_quote(quote_in, db, current_user=artist)
    declined = api_quote.decline_quote(quote.id, db, current_user=client)
