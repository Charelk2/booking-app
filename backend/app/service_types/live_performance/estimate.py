...
def _estimate_sound_cost_contextual(
    *,
    ...
    try:
        ...
        cost = estimate_sound_service_total(
            provider.details or {},
            guest_count=int(guest_count or 0),
            venue_type=(venue_type or "indoor"),
            stage_required=bool(stage_required),
            stage_size=stage_size,
            lighting_evening=bool(lighting_evening),
            upgrade_lighting_advanced=bool(upgrade_lighting_advanced),
            rider_units=rider_units,
            backline_requested=backline_requested,
        )
...
# Aliases exported via __all__ for callers expecting the old name
normalize_rider_for_pricing = _normalize_rider_for_pricing
